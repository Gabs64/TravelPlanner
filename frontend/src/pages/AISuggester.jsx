...
const AISuggester = () => {
      ...
      try {
        ...
        if (validResults.length > 2) {
          ...
          validResults = validResults.filter(r => 
            Math.abs(r.lat - medianLat) < 4.0 && Math.abs(r.lng - medianLng) < 4.0
          );
        }
  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      // Prioritize explicit [MAP: ...] tags generated by AI, fallback to bold locations if none present
      const finalLocations = locations.length > 0 ? locations : boldLocations;
